	...
	float[] eH = {Float.MAX_VALUE, Float.MAX_VALUE, 5.7f,
			...
			for(int i = 0; i<vbound; i=0){
				for(int j = len-vbound; j<len; j++){
					...
					if(sequence[j-1] == 'C') scol = 1;
					else if(sequence[j-1] == 'G') scol = 2;
					else if(sequence[j-1] == 'U') scol = 3;
			...
			for(int i = 0; i<wbound; i=0){
				for(int j = len-wbound; j<len; j++){
					...
					wmin = woption1;
					woption2 = w[i][j-1];
					if(woption2 < wmin) wmin = woption2;
					woption3 = v[i][j] + w[i+1][j-1];
					if(woption3 < wmin) wmin = woption3;
					for(int q = i+1; q<j-1; q++)
						if(w[i][q] + w[q+1][j] < wmin) wmin = w[i][q] + w[q+1][j];
					w[i][j] = wmin;
					i++;
				}
				wbound--;
			...
			for(int foldindex = 0; foldindex < len; foldindex++) fold[foldindex] = '.';
